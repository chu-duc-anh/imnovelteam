...
const AuthView: React.FC<AuthViewProps> = ({ initialMode, onBack, onLoginSuccess, onRegisterSuccess, siteSettings }) => {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMode('login');
      // Pre-fill login identifier for user convenience
      setLoginIdentifier(registerEmail);
      setLoginPassword('');
      
      // Reset registration form fields and state
      setRegisterUsername('');
      setRegisterEmail('');
      setRegisterPassword('');
      setConfirmPassword('');
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
            <div className="w-full max-w-md">
                <div className="text-center lg:text-left mb-10">
                    <h2 className="font-serif text-4xl font-bold text-primary-800 dark:text-primary-100">{mode === 'login' ? 'Chào mừng trở lại' : 'Tạo một tài khoản'}</h2>
                    <p className="text-primary-500 dark:text-primary-400 mt-2">{mode === 'login' ? 'Vui lòng đăng nhập để tiếp tục cuộc phiêu lưu của bạn.' : 'Bắt đầu trong vài giây.'}</p>
                </div>
