...
const SearchOverlay: React.FC<SearchOverlayProps> = ({ isOpen, onClose, onSelectStory }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            // Delay focus to allow for the transition animation
    ...
    const highlightMatch = (text: string, term: string): React.ReactNode => {
        ...
        return text.split(regex).map((part, index) => 
            regex.test(part) ? <mark key={index} className="bg-yellow-300 dark:bg-yellow-500 text-black rounded px-0.5 py-px">{part}</mark> : part
        );
    };
    ...
    return (
        <div className="fixed inset-0 bg-primary-950/70 backdrop-blur-sm z-[100] flex justify-center p-4 pt-[10vh]" onClick={onClose}>
            <div className="w-full max-w-2xl bg-primary-100 dark:bg-primary-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-in-down" onClick={e => e.stopPropagation()}>
                <header className="flex items-center p-2 border-b border-primary-300 dark:border-primary-700">
                    <div className="relative flex-grow">
                        ...
                        <input
                            ...
                            className="w-full pl-11 pr-4 py-3 bg-transparent border-0 rounded-lg focus:outline-none focus:ring-0 text-lg text-primary-900 dark:text-primary-100 placeholder-primary-500"
                        />
                         {searchTerm && (
                            <button onClick={() => { setSearchTerm(''); setResults([]); inputRef.current?.focus(); }} className="absolute inset-y-0 right-0 flex items-center px-4 text-primary-500 hover:text-primary-700">
                ...
                <div className="flex-grow overflow-y-auto">
                    {isLoading && <div className="p-8 flex justify-center"><LoadingSpinner message="Searching..." /></div>}
                    {!isLoading && searchTerm.trim().length >= 2 && results.length === 0 && (
                        <div className="p-8 text-center text-primary-500">Không tìm thấy kết quả cho "{searchTerm}".</div>
                    )}
                     {!isLoading && searchTerm.trim().length < 2 && (
                        <div className="p-8 text-center text-primary-500">Nhập ít nhất 2 ký tự để tìm kiếm.</div>
                    )}
                    {results.length > 0 && (
                        <ul>
                            <li className="px-4 py-3 border-b border-primary-200 dark:border-primary-800">
                                <button className="w-full text-left flex items-center gap-3 text-sm text-primary-600 dark:text-primary-300 hover:text-secondary-600 dark:hover:text-secondary-400">
                                    ...
                                    <span>Tìm kiếm nâng cao cho <strong>{searchTerm}</strong></span>
                                </button>
                            </li>
                            {results.map(story => (
                                <li key={story.id} className="border-b border-primary-200 dark:border-primary-800 last:border-b-0">
